...
function renderProcurementTab(day){
  ...
  return `<section class="menu-dashboard-section"><div class="menu-proc-hero"><div><div class="menu-proc-hero__title">智慧食材採購清單</div><div class="menu-proc-hero__sub">上方輸入供餐人數，下方會直接帶入當日菜單對應的食材與建議採購量。</div></div><label class="menu-proc-hero__count"><span>供餐人數</span><input type="number" min="1" step="1" value="${menuEscape(menuFormatAmount(servings,0))}" onchange="setMenuDashboardServings(this.value)"></label></div><div class="menu-proc-board"><div class="menu-proc-board__head"><span>食材</span><span>必須使用量（淨重）</span><span>預估耗損量</span><span>建議採購量（毛重）</span></div>${items.length ? items.map(item => renderProcurementRow(item)).join('') : `<div class="menu-panel-empty is-inline"><h3>尚未建立採購試算對應</h3><p>請在菜單管理勾選「採購試算」，並填寫每人用量與耗損率。</p></div>`}</div>${unmapped.length ? `<div class="menu-warning-box"><div class="menu-warning-box__title">以下菜色尚未建立採購試算對應</div><div class="menu-warning-box__list">${unmapped.map(v => `<span>${menuEscape(v.meal_label || '')}｜${menuEscape(v.dish_name || '')}</span>`).join('')}</div></div>` : ''}<div class="menu-proc-footnote">${menuEscape(`採購量計算＝(每人淨量 × ${menuFormatAmount(servings,0)} 人) ÷ (1 - 耗損率)。`)}</div>${groupHtml}</section>`;
}
